      ...
      <div className="bg-tema-superficie border border-tema-linha rounded-2xl w-full max-w-md shadow-xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-tema-linha">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-tema-tinta">Ajustar Estoque</h2>
              <p className="text-xs text-tema-apagado">{item.descricao}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Info atual */}
          <div className="flex items-center justify-between p-3 bg-tema-contraste/[0.02] border border-tema-linha rounded-xl">
            <div>
              <p className="text-xs text-tema-apagado">Codigo</p>
              <p className="text-sm font-mono text-tema-tinta">{item.codigo}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-tema-apagado">Estoque Atual</p>
              <p className="text-2xl font-bold text-tema-tinta">{formatNumber(item.quantidadeAtual)}</p>
              <p className="text-xs text-tema-apagado">{item.unidade}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-tema-apagado">Minimo</p>
              <p className="text-sm text-tema-suave">{formatNumber(item.quantidadeMinima)}</p>
            </div>
          </div>

          {/* Tipo de operacao */}
          <div>
            <label className="block text-sm font-medium text-tema-suave mb-2">Tipo de Operacao *</label>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      tipo === op.value ? `${op.bg} ${op.cor}` : 'border-tema-linha text-tema-suave hover:text-tema-tinta hover:border-tema-linha-forte'
                    )}
                  >
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="w-10 h-10 rounded-lg bg-tema-contraste/[0.04] hover:bg-tema-contraste/[0.08] text-tema-tinta font-bold text-lg flex items-center justify-center"
              >
                -
              </button>
              <input
              ...
              <span className="text-tema-apagado text-sm w-8">{item.unidade}</span>
            </div>
          </div>
          ...
          {quantidade > 0 && (
            <div className={cn(
              ...
              <span className="text-sm text-tema-suave">Nova quantidade:</span>
              <span className={cn(
                'text-xl font-bold',
          ...
          {/* Motivo */}
          <div>
            <label className="block text-sm font-medium text-tema-suave mb-1.5">Motivo *</label>
            <input
              value={motivo}
