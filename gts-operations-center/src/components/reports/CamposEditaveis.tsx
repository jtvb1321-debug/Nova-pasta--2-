...
export function CampoTexto({ value, onChange, className, placeholder }: CampoTextoProps) {
  return (
    <input
      ...
      className={cn(
        'bg-transparent border-b border-transparent hover:border-tema-linha-forte focus:border-orange-500 outline-none w-full transition-colors',
        className
      )}
    />
  )
...
export function BotaoRemoverLinha({ onClick, title = 'Remover do relatorio' }: { onClick: () => void; title?: string }) {
  return (
    <button
      onClick={onClick}
      title={title}
      className="text-tema-apagado hover:text-red-700 transition-colors flex-shrink-0 p-0.5"
    >
