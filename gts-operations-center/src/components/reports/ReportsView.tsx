      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={cn('text-sm font-medium', tipo === r.id ? 'text-tema-tinta' : 'text-tema-suave')}>{r.title}</p>
                    <p className="text-xs text-tema-apagado truncate">{r.description}</p>
                  </div>
          ...
          <div className="gts-card space-y-4">
            ...
            {tipo === 'diario' ? (
              <div>
                <label className="block text-xs font-medium text-tema-suave mb-1.5">Data</label>
                <input
                  type="date"
                  value={dataDiario}
              ...
              <div>
                <label className="block text-xs font-medium text-tema-suave mb-1.5">Mes</label>
                <input
                  type="month"
                  value={mesCancelados}
                  ...
                  className="w-full gts-input"
                />
                <p className="text-xs text-tema-apagado mt-1.5">Consulta direto na API do IXC.</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full gts-input"
                />
                <p className="text-xs text-tema-apagado mt-1.5">Chamados e Qualidade/SLA saem juntos no mesmo PDF, para o mes escolhido.</p>
              </div>
            ) : tipo === 'comercial' ? (
              <>
                <div>
                  <label className="block text-xs font-medium text-tema-suave mb-1.5">Periodo (rotulo no PDF)</label>
                ...
                {periodo === 'personalizado' && (
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-medium text-tema-suave mb-1.5">Data Inicio</label>
                      <input type="date" value={dataInicio} onChange={e => setDataInicio(e.target.value)} className="w-full gts-input" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-tema-suave mb-1.5">Data Fim</label>
        ...
        <div className="lg:col-span-2">
          <div className="gts-card h-full">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-base font-semibold text-tema-tinta">{relatorioAtual.title}</h2>
                <p className="text-xs text-tema-apagado">{relatorioAtual.description}</p>
              </div>
            </div>

            {!dadosCarregados ? (
              <div className="text-center py-16">
                <relatorioAtual.icon className="w-10 h-10 text-tema-apagado mx-auto mb-3" />
                <p className="text-tema-suave font-medium">Carregue os dados para revisar</p>
                <p className="text-tema-apagado text-sm mt-1">Clique em &quot;Carregar Dados&quot; ao lado. Voce podera corrigir textos, numeros e remover linhas antes de gerar o PDF.</p>
              </div>
            ) : tipo === 'chamados_qualidade' && dadosChamadosQualidade ? (
              <div className="space-y-6">
                <div className="grid grid-cols-4 gap-3">
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Total</p>
                    <p className="text-xl font-bold text-blue-700">{kpisChamados.total}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Finalizados</p>
                    <p className="text-xl font-bold text-emerald-700">{kpisChamados.finalizados}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Em Andamento</p>
                ...
                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Chamados do Periodo ({chamadosAtual.length})</p>
                  {chamadosAtual.length === 0 ? (
                    <p className="text-xs text-tema-apagado">Nenhum chamado no periodo.</p>
                  ) : (
                    <div className="overflow-x-auto max-h-80 overflow-y-auto">
                      <table className="w-full text-xs">
                        <thead className="sticky top-0 bg-tema-superficie">
                          <tr className="text-tema-apagado border-b border-tema-linha">
                        ...
                        <tbody>
                          {chamadosAtual.map((c: any, i: number) => (
                            <tr key={c.id ?? i} className="border-b border-tema-linha">
                              <td className="py-1 pr-2">
                                <CampoTexto
                                  value={c.cliente}
                                  onChange={v => setDadosChamadosQualidade(d => d && ({ ...d, chamados: atualizarItem(d.chamados, i, { cliente: v }) }))}
                                  className="text-tema-texto"
                                />
                              </td>
                              <td className="py-1 pr-2 text-tema-suave">{c.tipo}</td>
                              <td className="py-1 pr-2">
                                <CampoTexto
                                  value={c.cidade}
                                  onChange={v => setDadosChamadosQualidade(d => d && ({ ...d, chamados: atualizarItem(d.chamados, i, { cidade: v }) }))}
                                  className="text-tema-suave"
                                />
                              </td>
                              <td className="py-1 pr-2 text-tema-suave">{c.equipe?.nome || '-'}</td>
                              <td className="py-1 pr-2 text-tema-suave">{c.status}</td>
                              <td className="py-1">
                ...
                <div className="border-t border-tema-linha pt-4">
                  <p className="text-xs font-semibold text-tema-suave mb-2">Qualidade / SLA do Mes</p>
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-2.5 flex items-center justify-between">
                      <span className="text-[11px] text-tema-apagado">Reincidencias (qtd)</span>
                      <CampoNumero
                        ...
                        className="text-right text-tema-tinta font-bold w-16"
                      />
                    </div>
                    <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-2.5 flex items-center justify-between">
                      <span className="text-[11px] text-tema-apagado">Reincidencia (%)</span>
                      <CampoNumero
                    ...
                    <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-2.5 flex items-center justify-between">
                      <span className="text-[11px] text-tema-apagado">SLA resposta (%)</span>
                      <CampoNumero
                    ...
                    <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-2.5 flex items-center justify-between">
                      <span className="text-[11px] text-tema-apagado">SLA resolucao (%)</span>
                      <CampoNumero
                  ...
                  <div className="space-y-1 mb-4">
                    {dadosChamadosQualidade.qualidade.reincidencia.porCliente.length === 0 ? (
                      <p className="text-xs text-tema-apagado">Nenhum cliente reincidente no mes.</p>
                    ) : dadosChamadosQualidade.qualidade.reincidencia.porCliente.map((c: any, i: number) => (
                      <div key={i} className="flex items-center gap-2 text-xs bg-tema-contraste/[0.02] rounded px-2.5 py-1.5">
                        <CampoTexto
                          value={c.cliente}
                          ...
                          className="text-tema-suave flex-1"
                        />
                        <CampoNumero
                          value={c.quantidade}
                          ...
                          className="text-tema-apagado w-12 text-right"
                        />
                  ...
                  <p className="text-[11px] text-tema-apagado mb-1.5">Reincidencia por Tipo</p>
                  <div className="space-y-1 mb-4">
                    {Object.entries(dadosChamadosQualidade.qualidade.reincidencia.porTipo).map(([tipoChamado, v]: [string, any]) => (
                      <div key={tipoChamado} className="flex items-center gap-2 text-xs bg-tema-contraste/[0.02] rounded px-2.5 py-1.5">
                        <span className="text-tema-suave flex-1">{tipoChamado}</span>
                        <span className="text-tema-apagado">total</span>
                        <CampoNumero
                          value={v.total}
                          ...
                          className="text-tema-suave w-10 text-right"
                        />
                        <span className="text-tema-apagado">reinc.</span>
                        <CampoNumero
                  ...
                  <p className="text-[11px] text-tema-apagado mb-1.5">Evolucao dos Ultimos 6 Meses</p>
                  <div className="space-y-1">
                    {dadosChamadosQualidade.qualidade.evolucao.map((e: any, i: number) => (
                      <div key={i} className="flex items-center gap-2 text-xs bg-tema-contraste/[0.02] rounded px-2.5 py-1.5">
                        <span className="text-tema-suave w-16 flex-shrink-0">{e.mes}</span>
                        <span className="text-tema-apagado">chamados</span>
                        <CampoNumero
                          ...
                          className="text-tema-suave w-10 text-right"
                        />
                        <span className="text-tema-apagado">SLA%</span>
                        <CampoNumero
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-4 gap-3">
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Total de Itens</p>
                    <p className="text-xl font-bold text-blue-700">{kpisEstoque.total}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-red-500/20 rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Criticos</p>
                    <p className="text-xl font-bold text-red-700">{kpisEstoque.criticos}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Valor Total</p>
                    <p className="text-lg font-bold text-emerald-700">R$ {kpisEstoque.valorTotal.toFixed(2)}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Categorias</p>
                    <p className="text-xl font-bold text-tema-suave">{kpisEstoque.categorias}</p>
                  </div>
                </div>

                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Itens do Estoque ({itensEstoqueAtual.length})</p>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {itensEstoqueAtual.map((item: any, i: number) => (
                          <tr key={item.id ?? i} className="border-b border-tema-linha">
                            <td className="py-1 pr-2">
                              <CampoTexto value={item.descricao} onChange={v => setDadosEstoque(lst => lst && atualizarItem(lst, i, { descricao: v }))} className="text-tema-texto" />
                            </td>
                            <td className="py-1 pr-2">
                              <CampoNumero value={item.quantidadeAtual} onChange={v => setDadosEstoque(lst => lst && atualizarItem(lst, i, { quantidadeAtual: v }))} className="text-tema-suave w-16" />
                            </td>
                            <td className="py-1 pr-2">
                              <CampoNumero value={item.quantidadeMinima} onChange={v => setDadosEstoque(lst => lst && atualizarItem(lst, i, { quantidadeMinima: v }))} className="text-tema-suave w-16" />
                            </td>
                            <td className="py-1 pr-2">
                              <CampoNumero value={item.valorUnitario} step={0.01} onChange={v => setDadosEstoque(lst => lst && atualizarItem(lst, i, { valorUnitario: v }))} className="text-tema-suave w-20" />
                            </td>
                            <td className="py-1">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Total de Vendas</p>
                    <p className="text-xl font-bold text-blue-700">{kpisComercial.total}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Aprovadas</p>
                    <p className="text-xl font-bold text-emerald-700">{kpisComercial.aprovadas}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Faturamento</p>
                    <p className="text-lg font-bold text-emerald-700">R$ {kpisComercial.faturamento.toFixed(2)}</p>
                  </div>
                </div>

                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Ranking de Vendedores</p>
                  <div className="space-y-1">
                    {dadosComercial.ranking.map((v: any, i: number) => (
                      <div key={i} className="flex items-center gap-2 text-xs bg-tema-contraste/[0.02] rounded px-2.5 py-1.5">
                        <span className="text-tema-apagado w-6">{i + 1}o</span>
                        <CampoTexto value={v.nome} onChange={val => setDadosComercial(d => d && ({ ...d, ranking: atualizarItem(d.ranking, i, { nome: val }) }))} className="text-tema-texto flex-1" />
                        <CampoNumero value={v.totalVendas} onChange={val => setDadosComercial(d => d && ({ ...d, ranking: atualizarItem(d.ranking, i, { totalVendas: val }) }))} className="text-tema-suave w-14 text-right" />
                ...
                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Vendas ({vendasAtual.length})</p>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {vendasAtual.map((v: any, i: number) => (
                          <tr key={v.id ?? i} className="border-b border-tema-linha">
                            <td className="py-1 pr-2">
                              <CampoTexto value={v.clienteNome} onChange={val => setDadosComercial(d => d && ({ ...d, vendas: atualizarItem(d.vendas, i, { clienteNome: val }) }))} className="text-tema-texto" />
                            </td>
                            <td className="py-1 pr-2">
                              <CampoNumero value={v.valor} step={0.01} onChange={val => setDadosComercial(d => d && ({ ...d, vendas: atualizarItem(d.vendas, i, { valor: val }) }))} className="text-tema-suave w-20" />
                            </td>
                            <td className="py-1 pr-2 text-tema-suave">{v.status}</td>
                            <td className="py-1">
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-tema-tinta">Relatorio Diario Operacional</p>
                  <span className="text-xs text-tema-apagado">{new Date(dataDiario + 'T12:00:00').toLocaleDateString('pt-BR')}</span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {([
                    ...
                    <div key={campo} className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                      <p className="text-[11px] text-tema-apagado mb-1">{label}</p>
                      <CampoNumero
                        value={dadosDiario.kpis[campo]}
                        onChange={v => setDadosDiario((d: any) => ({ ...d, kpis: { ...d.kpis, [campo]: v } }))}
                        className="text-xl font-bold text-tema-tinta"
                      />
                    </div>
                  ))}
                </div>
                ...
                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Atendimentos e OS Concluidas ({dadosDiario.atendimentos.length})</p>
                  {dadosDiario.atendimentos.length === 0 ? (
                    <p className="text-xs text-tema-apagado">Nenhum atendimento finalizado no dia.</p>
                  ) : (
                        ...
                        <tbody>
                          {dadosDiario.atendimentos.map((a: any, i: number) => (
                            <tr key={i} className="border-b border-tema-linha">
                              <td className="py-1 pr-2">
                                <CampoTexto value={a.cliente} onChange={v => setDadosDiario((d: any) => ({ ...d, atendimentos: atualizarItem(d.atendimentos, i, { cliente: v }) }))} className="text-tema-suave" />
                              </td>
                              <td className="py-1 pr-2 text-tema-suave">{a.equipeNome}</td>
                              <td className="py-1 pr-2 text-tema-suave">{a.tipoLabel}</td>
                              <td className="py-1 pr-2 text-tema-suave">{a.tma}</td>
                ...
                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Produtividade das Equipes de Campo</p>
                  <div className="space-y-1.5">
                    {dadosDiario.produtividadeEquipes.map((e: any, i: number) => (
                      <div key={i} className="flex items-center gap-2 text-xs bg-tema-contraste/[0.02] rounded-lg px-3 py-2">
                        <span className="text-tema-suave font-medium flex-shrink-0 w-28 truncate">{e.equipeNome}</span>
                        <CampoNumero value={e.osFinalizadas} onChange={v => setDadosDiario((d: any) => ({ ...d, produtividadeEquipes: atualizarItem(d.produtividadeEquipes, i, { osFinalizadas: v }) }))} className="text-tema-apagado w-12 text-center" />
                        <span className="text-tema-apagado">OS</span>
                ...
                {/* 4. Feedback */}
                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Feedback dos Clientes (WhatsApp)</p>
                  <div className="grid grid-cols-3 gap-3">
                    {([
                      ['Enviadas', 'enviados'],
                      ...
                      <div key={campo} className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-2.5">
                        <p className="text-[10px] text-tema-apagado mb-1">{label}</p>
                        <CampoNumero
                          value={dadosDiario.feedback[campo]}
                          onChange={v => setDadosDiario((d: any) => ({ ...d, feedback: { ...d.feedback, [campo]: v } }))}
                          className="text-base font-bold text-tema-tinta"
                        />
                      </div>
                    ))}
                  </div>
                ...
                {/* 5. Ponto */}
                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Registro de Ponto das Equipes</p>
                  {dadosDiario.pontoPorEquipe.length === 0 ? (
                    <p className="text-xs text-tema-apagado">Nenhum registro de ponto encontrado para o dia.</p>
                  ) : (
                    <div className="space-y-3">
                        ...
                        <div key={eq.equipeId ?? ei}>
                          ...
                          <div className="space-y-1">
                            {eq.registros.map((r: any, ri: number) => (
                              <div key={ri} className="flex items-center gap-2 text-xs bg-tema-contraste/[0.02] rounded px-2.5 py-1.5">
                                <CampoTexto
                                  ...
                                  className="text-tema-suave flex-1"
                                />
                                <span className="text-tema-apagado">{r.entrada} - {r.saida}</span>
                ...
                {/* 6. Estoque */}
                <div>
                  <p className="text-xs font-semibold text-tema-suave mb-2">Estoque Embarcado por Equipe</p>
                  {dadosDiario.estoquePorVeiculo.length === 0 ? (
                    <p className="text-xs text-tema-apagado">Nenhum item carregado em veiculo de equipe no momento.</p>
                  ) : (
                    <div className="space-y-1">
                      {dadosDiario.estoquePorVeiculo.map((e: any, i: number) => (
                        <div key={i} className="flex items-center gap-2 text-xs bg-tema-contraste/[0.02] rounded px-2.5 py-1.5">
                          <span className="text-tema-suave flex-shrink-0 w-32 truncate">{e.equipeNome}{e.veiculoPlaca ? ` - ${e.veiculoPlaca}` : ''}</span>
                          <CampoTexto value={e.item} onChange={v => setDadosDiario((d: any) => ({ ...d, estoquePorVeiculo: atualizarItem(d.estoquePorVeiculo, i, { item: v }) }))} className="text-tema-suave flex-1" />
                          <CampoNumero value={e.quantidade} onChange={v => setDadosDiario((d: any) => ({ ...d, estoquePorVeiculo: atualizarItem(d.estoquePorVeiculo, i, { quantidade: v }) }))} className="text-tema-apagado w-12 text-right" />
                ...
                <div className="border-t border-tema-linha pt-4">
                  <p className="text-xs font-semibold text-tema-suave mb-2">Resumo Executivo</p>
                  <div className="space-y-1.5">
                    {([
                      ...
                      <div key={campo} className="flex items-center gap-2 text-xs">
                        <span className="text-tema-apagado flex-shrink-0">{label}:</span>
                        <CampoTexto
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-tema-tinta">Cancelamentos - Direto da API do IXC</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-tema-contraste/[0.02] border border-red-500/20 rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Total de Cancelamentos</p>
                    <p className="text-xl font-bold text-red-700">{dadosCancelados.cancelados.length}</p>
                  </div>
                  <div className="bg-tema-contraste/[0.02] border border-tema-linha rounded-lg p-3">
                    <p className="text-[11px] text-tema-apagado">Principal Motivo</p>
                    <p className="text-sm font-bold text-tema-tinta truncate">
                ...
                {dadosCancelados.cancelados.length === 0 ? (
                  <p className="text-tema-apagado text-sm text-center py-8">Nenhum cancelamento encontrado no IXC para esse mes.</p>
                ) : (
                  <div>
                    <p className="text-xs font-semibold text-tema-suave mb-2">Clientes Cancelados no Periodo ({dadosCancelados.cancelados.length})</p>
                    <div className="overflow-x-auto max-h-96 overflow-y-auto">
                      <table className="w-full text-xs">
                        <thead className="sticky top-0 bg-tema-superficie">
                          <tr className="text-tema-apagado border-b border-tema-linha">
                        ...
                        <tbody>
                          {dadosCancelados.cancelados.map((c: any, i: number) => (
                            <tr key={c.contratoId ?? i} className="border-b border-tema-linha">
                              <td className="py-1 pr-2">
                                <CampoTexto value={c.clienteNome} onChange={v => setDadosCancelados((d: any) => ({ ...d, cancelados: atualizarItem(d.cancelados, i, { clienteNome: v }) }))} className="text-tema-suave" />
                              </td>
                              <td className="py-1 pr-2">
                                <CampoTexto value={c.cidade} onChange={v => setDadosCancelados((d: any) => ({ ...d, cancelados: atualizarItem(d.cancelados, i, { cidade: v }) }))} className="text-tema-suave" />
                              </td>
                              <td className="py-1">
                                <CampoTexto value={c.motivoResumo} onChange={v => setDadosCancelados((d: any) => ({ ...d, cancelados: atualizarItem(d.cancelados, i, { motivoResumo: v }) }))} className="text-tema-suave" />
                              </td>
                              <td className="py-1">
            ...
            {dadosCarregados && (
              <p className="text-xs text-tema-apagado text-center mt-4">
                Corrija o que precisar acima e clique em <strong className="text-tema-tinta">Gerar e Baixar PDF</strong>. As correcoes valem so para este PDF.
              </p>
            )}
          </div>
        </div>
